...
async function extractAnnotations(cssFiles, options = {}) {
  ...
  for (const cssFile of cssFiles) {
    try {
      ...
      const result = await postcss([cssAnnotationsPlugin(pluginOptions)])
        .process(css, { from: relativePath });
      
      // 抽出データを取得
      const dataMessage = result.messages.find(msg => msg.type === 'css-annotations-data');
      if (!dataMessage) {
        console.warn(`警告: ${relativePath} からアノテーションデータが抽出できませんでした`);
        continue;
      }
      ...
      if (data.meta.totalClasses > 0) {
        console.log(`${relativePath} から ${data.meta.totalClasses} 個のクラスを抽出しました`);
      }
    } catch (error) {
...
function convertComponentTypesFormat(data) {
  ...
  for (const [component, classes] of Object.entries(data.componentTypes)) {
    ...
    let category = 'other';
    for (const className of classes) {
      if (data.classDescriptions[className] && data.classDescriptions[className].category) {
  ...
  for (const component in data.componentVariants) {
    ...
    for (const [variant, className] of Object.entries(variants)) {
      if (variant !== 'base') { // baseはbaseClassesに含まれるのでスキップ
...
export async function processCssFiles(options = {}) {
  ...
  // CSSファイルを検索
  const cssFiles = globSync(opts.cssPattern)
  ...
  console.log(`合計 ${extractedData.meta.totalClasses} 個のクラスを抽出しました`);
  ...
  if (opts.generateDocs) {
    ...
    await docGenerator.generateAstroDocPages(extractedData, {
      ...config,
      fileOperations: {
        ...config.fileOperations,
        force: opts.force // force 設定はoptsのものを優先
  ...
  if (opts.generateComponents) {
    ...
    const useComponentForce = opts.force || config.fileOperations.forceByType.components;
