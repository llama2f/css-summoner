/**
 * @file src/css-summoner/ui/components/selectors/SizeSelector.tsx
 ...
 *
 * @props {object} props
 *   @prop {Array<SizeOption>} [sizes=[]] - 選択可能なサイズオプションの配列。
 *   @prop {string} selectedSize - 現在選択されているサイズクラスの値 (例: 'btn-sm', 'lg')。
 *   @prop {(value: string) => void} onSelect - サイズが選択されたときに呼び出されるコールバック関数。
 *
...
// Propsで受け取るサイズオプションの型定義
interface SizeOption {
  value: string;
  label: string;
}

// SizeSelectorコンポーネントのPropsの型定義
interface SizeSelectorProps {
  sizes?: SizeOption[]; // オプショナルに変更し、デフォルト値をコンポーネント内で設定
  selectedSize: string;
...
const SizeSelector: React.FC<SizeSelectorProps> = ({
   ...
   * @returns {SizeOption[]} 値が空でないサイズオプションの配列
   */
   ...
   * @param {React.MouseEvent<HTMLButtonElement>} e - クリックイベントオブジェクト
