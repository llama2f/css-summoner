...
} from '@/css-summoner/configs/colors'
 ...
 */
const ColorPicker = ({
	initialColor = defaultCustomColor.mainColor,
	onColorChange,
	isOpen,
	onClose,
}) => {
	...
	const [autoTextColor, setAutoTextColor] = useState(
		defaultCustomColor.autoTextColor
	)
	const [autoBorderColor, setAutoBorderColor] = useState(
		defaultCustomColor.autoBorderColor
	)
		...
		<div className='color-picker-container fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
			<div className='color-picker-panel bg-white p-6 rounded-lg shadow-lg max-w-md w-full'>
				...
				<div className='mb-4'>
					...
					<div
						className={`flex gap-2 items-center ${autoTextColor ? 'opacity-50' : ''}`}
					>
						<input
							type='color'
							value={textColor}
