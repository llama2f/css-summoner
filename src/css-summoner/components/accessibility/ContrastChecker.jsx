...
import {
	calculateContrast,
	getContrastRating,
} from '@/css-summoner/utils/contrastUtils'
		...
		<div className='contrast-checker mt-2 p-3 border rounded bg-white shadow-sm'>
			<div className='flex items-center justify-between'>
				...
				<div className='flex items-center gap-2'>
					...
					<span
						className={`px-2 py-0.5 rounded-full text-xs ${wcagLevel.level === 'Fail' ? 'bg-red-500 text-white' : 'bg-green-600 text-white'}`}
					>
						{wcagLevel.level}
					</span>
				</div>
			...
			{showDetails && (
				<div className='mt-3 border-t pt-3'>
					{/* サンプルテキスト */}
					<div
						className='mt-2 p-3 rounded relative'
						style={{
							backgroundColor,
							color: textColor,
					...
					<div className='mt-3 text-xs text-gray-600 bg-gray-50 p-2 rounded'>
						...
						<div className='flex justify-between mb-1'>
							...
							<span
								className={
									contrastRatio >= 3 ? 'text-green-600' : 'text-red-600'
								}
							>
						...
						<div className='flex justify-between'>
							...
							<span
								className={
									contrastRatio >= 7 ? 'text-green-600' : 'text-red-600'
								}
							>
			...
			{wcagLevel.level === 'Fail' && (
				<div className='mt-2 text-sm text-red-600'>
					<p>
						コントラストが不十分です。読みやすさのために色の組み合わせを調整してください。
					</p>
				</div>
			)}
		</div>
