	...
	safeWriteFile: (filePath, content, options = {}) => {
		...
		try {
			if (fs.existsSync(filePath) && !force) {
				...
				logger.info(
					'上書きを強制するには --force オプションを使用してください。'
				)
				return false
			}
...
/**
 * キャッシュを回避してモジュールを動的にインポートする
 * @param {string} modulePath モジュールのパス
 * @returns {Promise<any>} インポートされたモジュール
 */
export async function dynamicImport(modulePath) {
	const absolutePath = path.resolve(modulePath)
	const cacheBustingPath = `${absolutePath}?update=${Date.now()}`
	try {
		logger.verbose(`動的インポート実行: ${cacheBustingPath}`)
		return await import(cacheBustingPath)
	} catch (error) {
		logger.error(`動的インポートに失敗しました: ${modulePath}`, error)
		throw error
	}
}
