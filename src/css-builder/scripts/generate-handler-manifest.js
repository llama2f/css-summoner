...
// スクリプトの場所 (src/css-builder/scripts) からプロジェクトルート (css-builder/) を特定
const projectRoot = path.resolve(__dirname, '../../../');
// src ディレクトリのパス
const srcDir = path.resolve(projectRoot, 'src');

// ハンドラーファイルが存在するディレクトリ
const handlersDir = path.resolve(projectRoot, 'src/css-builder/templates/handlers/auto');
// 生成されるマニフェストファイルのパス
const manifestPath = path.resolve(projectRoot, 'src/css-builder/configs/handler-manifest.json');
...
async function generateManifest() {
  ...
  console.log(`Searching for handler files in: ${path.relative(projectRoot, handlersDir)}`);
  ...
  if (handlerFiles.length === 0) {
    ...
    handlerFiles.forEach(file => console.log(`  - ${path.relative(projectRoot, file)}`));
  }

  const manifest = {};

  for (const filePath of handlerFiles) {
    // 実行時の import() で使用するパスを計算 (例: '/src/css-builder/templates/handlers/auto/button.jsx')
    // src ディレクトリからの相対パスを取得し、先頭に '/' を付ける
    const relativePathFromSrc = path.relative(srcDir, filePath).replace(/\\/g, '/');
    ...
    try {
      ...
      if (match && match[1]) {
        ...
        try {
          ...
          console.warn(`⚠️ Warning: Skipping ${fileName}. Could not parse metadata object.`);
        ...
        if (metadata && typeof metadata === 'object' && metadata.type) {
          ...
          if (manifest[handlerType]) {
            console.warn(`⚠️ Warning: Duplicate handler type "${handlerType}" found.`);
          ...
          console.log(`  👍 Processed: ${fileName} (Type: ${handlerType}, Path: ${importPath})`);
        } else {
          console.warn(`⚠️ Warning: Skipping ${fileName}. Parsed metadata is invalid or missing 'type' property.`);
          console.warn(`   Parsed metadata:`, metadata);
        }
      } else {
        console.warn(`⚠️ Warning: Skipping ${fileName}. Could not find 'export const metadata = { ... }' pattern.`);
      }
    } catch (error) {
  ...
  try {
    ...
    console.error(`❌ Error writing manifest file: ${path.relative(projectRoot, manifestPath)}`);
    console.error(error);
    process.exit(1);
  }
}
